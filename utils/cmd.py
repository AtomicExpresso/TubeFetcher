...
class AppCmd:
    ...
    def add_video(self) -> None:
        if self.app.is_downloading:
            Utils.create_dialog_error_notification(
                "Cant add new videos because a download is in progress"
            )
            pass
        elif len(self.app.vid_queue) >= 10:  # limited for performence reasons...
            Utils.create_dialog_error_notification(
                "The video queue is full (limit: 10). Please download the current videos before adding more."
            )
            pass
        else:
            try:
    ...
    def select_folder(self) -> None:
        ...
        try:
            ...
            if folder_selected:
                ...
                messagebox.showinfo(
                    "File path updated", f"Download folder is now:\n {folder_selected}"
                )
                Utils.save_settings_data()
        except:
            self.app.widgets.error_txt.configure(text="Incorrect file path")
            self.app.widgets.error_txt.grid(
                row=0, column=0, columnspan=2, pady=(120, 0), padx=(0, 0)
            )
            Utils.create_dialog_error_notification(
                "Unable to change file path, did you give the app permissions?"
            )
            raise ValueError(
                "Unable to change file path, did you give the app permissions?"
            )
    ...
    def set_progress(self) -> None:
        ...
        self.app.widgets.download_progress_txt.configure(
            text=f"{self.app.vid_dl_count} of {len(self.app.vid_queue)} completed"
        )
    ...
    def clear_main_frame(self) -> None:
        if self.app.is_downloading:
            Utils.create_dialog_notfication(
                "Cant clear frame because a download is in progress"
            )
            pass
        else:
            try:
                ...
                Utils.create_dialog_error_notification(
                    "An error occured while clearing widgets"
                )
    ...
    def check_vid_resoultion(self, i) -> None:
        try:
            ...
            self.app.is_downloading = False
            Utils.create_dialog_error_notification(
                "An error occured while fetching video stream, is your internet connected?"
            )
            raise ValueError(
                "An error occured while fetching video stream, is your internet connected?"
            )
    ...
    def set_single_download_option(self, txt, i) -> None:
        if self.app.is_downloading:
            Utils.create_dialog_notfication(
                "Cant change options because a download is in progress"
            )
            pass
        else:
            cur = self.app.vid_queue[i]
    ...
    def destroy_progress_frame(self) -> None:
        if (
            self.app.frames.progress_frame.winfo_exists()
            and not self.app.is_downloading
        ):
    ...
    def throw_progress_error(self, msg: str) -> None:
        ...
        self.app.widgets.error_txt.grid(
            row=0, column=0, columnspan=2, pady=(10, 0), padx=(0, 0)
        )
    ...
    def download_btn(self) -> None:
        if self.app.is_downloading:
            Utils.create_dialog_notfication(
                "Unable to download because a download is already in progress"
            )
            pass
        ...
        try:
            ...
            Utils.create_dialog_error_notification(
                "an error occured while building progress bars"
            )
            raise ValueError("an error occured while building progress bars")
        try:
            # bottom progress frame
            self.app.frames.progress_frame.grid(
                row=2, column=0, columnspan=3, sticky="nsew"
            )
            self.app.widgets.download_progress_txt.grid(
                row=0, column=0, pady=(10, 0), padx=(20, 0)
            )
            self.app.widgets.download_progress_bar.grid(
                row=1, column=0, columnspan=2, padx=(20, 20), pady=(0, 10), sticky="ew"
            )
            self.app.widgets.error_txt.configure(text="")
            ...
            for i, video in enumerate(self.app.vid_queue):
                try:
                    ...
                    if cur_filter:
                        ...
                        cur_stream.get_highest_resolution().download(
